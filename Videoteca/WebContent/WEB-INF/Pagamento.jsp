...
<% Utente cliente = (Utente) request.getAttribute("utente");%>
<table class="table table-bordered table-striped text-center">
<tr>
<th>titolo</th>
</tr>
  <%for(int i=0; i < cliente.getCarrello().getFilmCarrello().size();i++){%>
            <tr>
                <td><%=cliente.getCarrello().getFilmCarrello().get(i).getTitolo() %></td>
                
             	<td>
             	<form method="POST" action="CarrelloFilm?val=rimuovi">      	
				<input type="hidden" value=<%=cliente.getCarrello().getFilmCarrello().get(i).getId()%> name="filmId">
    ...
    <form method="POST" action="CarrelloFilm?val=pagamento" method="post">
    	<input type="hidden" name="pagamento" value=<%= cliente%>  />
